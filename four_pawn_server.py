#!/usr/bin/env python3
"""
Tablebase Server for Four Pawn Chess
...
# Import position encoding from validator
from four_pawn_validator import Position, WHITE, BLACK, WHITE_WIN, BLACK_WIN, DRAW, FILES
...
class TablebaseHandler(BaseHTTPRequestHandler):
    ...
    def serve_html(self):
        ...
        try:
            with open('four_pawn_game.html', 'rb') as f:
                content = f.read()
        ...
        try:
            ...
            for rank in range(8):
                for file in range(8):
                    ...
                    if piece == 0:  # WHITE
                        # Adjust for c-f files (file indices 2-5 map to 0-3)
                        sq = rank * FILES + (file - 2)
                        if 0 <= sq < 32:
                            white_pawns |= (1 << sq)
                    elif piece == 1:  # BLACK
                        sq = rank * FILES + (file - 2)
    ...
    def get_stats(self):
        ...
        if TablebaseHandler.tablebase is None:
            ...
            stats = {
                ...
                'size_mb': os.path.getsize('four_pawn_tablebase.pkl') / (1024 * 1024)
                           if os.path.exists('four_pawn_tablebase.pkl') else 0
            }
...
def main():
    ...
    print("=" * 60)
    print("Four Pawn Chess - Tablebase Server")
    print("=" * 60)

    # Load tablebase
    TablebaseHandler.tablebase = load_tablebase('four_pawn_tablebase.pkl')

    if TablebaseHandler.tablebase is None:
        print("\nERROR: Could not load tablebase!")
        print("Please run four_pawn_validator.py first to generate the tablebase.")
        return
