...
pair<bool, int8_t> is_terminal(const Position& pos) {
    ...
    // Check for promotion (pawn already on promotion rank)
    for (int file = 0; file < FILES; file++) {
    ...
    // Check if position can promote (pawn one move away with clear path AND it's that player's turn)
    if (pos.white_to_move) {
        // White to move: check if white can promote
        for (int file = 0; file < FILES; file++) {
            if (pos.get_piece(file, 6) == 0 && pos.get_piece(file, 7) == -1) {
                return {true, RESULT_WHITE_WIN};  // White can promote next move
            }
        }
    } else {
        // Black to move: check if black can promote
        for (int file = 0; file < FILES; file++) {
            if (pos.get_piece(file, 1) == 1 && pos.get_piece(file, 0) == -1) {
                return {true, RESULT_BLACK_WIN};  // Black can promote next move
            }
        }
    }
...
unordered_map<__uint128_t, Evaluation, KeyHash> retrograde_analysis(
    ...
    cout << "  Total positions: " << canonical_positions.size() << endl;
